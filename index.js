...
const askUser = async () => {
  ...
  rl.question('Нажмите ENTER чтобы отправить: ', message => {
    // Broadcast to peers
    for (let id in peers) {
...
;(async () => {
  ...
  sw.on('Подключение', (conn, info) => {
    // Connection id
    const seq = connSeq

    const peerId = info.id.toString('hex')
    log(`Вошел пользователь с ID ${peerId}`)
    ...
    conn.on('close', () => {
	  ...
	  log(`Пользователь с ID ${peerId} вышел`)
